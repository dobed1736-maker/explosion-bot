...
def guardar_senal(
  ...
  try:
    ...
    conn.close()
    print(
        f'💾 Señal de {symbol} guardada exitosamente en PostgreSQL (ID:'
        f' {inserted_id})'
    )
  except Exception as e:
...
def actualizar_estado_senal(
  ...
  """Actualiza el estado de una señal (p.ej. EJECUTADA, ABIERTA, GANADA, PERDIDA, CANCELADA).

  Se puede buscar por ID o por Symbol (actualizando la última señal activa de ese
  symbol).
  """
  conn = get_connection()
  if not conn:
  ...
  try:
    ...
    if id_senal:
      ...
      # Busca la orden más reciente de este símbolo que AÚN NO haya finalizado (NO GANADA, PERDIDA, etc.)
      cursor.execute(
          """
                UPDATE senales 
                SET estado = %s,
                ...
                WHERE id = (
                    ...
                    WHERE symbol = %s AND estado NOT IN ('GANADA', 'PERDIDA', 'CANCELADA', 'CERRADA')
    ...
    conn.close()
    print(
        f"🔄 Estado de señal ({symbol or f'ID {id_senal}'}) actualizado a"
        f" '{nuevo_estado}' en BD."
    )
  except Exception as e:
